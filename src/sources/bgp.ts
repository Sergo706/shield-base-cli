...
const tempASNJson = path.resolve(__dirname, './temp_asn_data.json');
...
export async function getBGPAndASN(userAgent: string, outputPath: string) {
    ...
    try {
        ...
        console.log(`[ASN/BGP] SUCCESS: Loaded ${String(asnDictionary.size)} ASN definitions.`);
        console.log("[ASN/BGP] Fetching Routing Table...");
        ...
        console.log("[ASN/BGP] Mapping CIDR to ASN data...");
        ...
        if (results.length > 0) {
            console.log('[ASN/BGP] SAMPLE:', results[0]);
        }

        console.log(`[ASN/BGP] Writing ${String(results.length)} enriched entries to temporary JSON...`);
        fs.writeFileSync(tempASNJson, results.join('\n'), 'utf-8');
        console.log(`[ASN/BGP] Compiling MMDB with mmdbctl to ${output}...`);
        
        const cmd = `mmdbctl import --in ${tempASNJson} --out ${output}`;
        const convert = await run(cmd);

        if (convert.stdout) console.log(`[ASN/BGP] mmdbctl: ${convert.stdout.toString().trim()}`);
        console.log(`[ASN/BGP] COMPLETED: Successfully compiled ASN MMDB to ${output}\n`);
